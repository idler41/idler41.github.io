...
>RuoYi是一款开源的优秀的后台管理系统。作为流行的启动项目模板，其优势是内置了丰富的系统功能、漂亮的前端ui、在线简洁的代码生成器。但大量无封装重复的前后端代码，不优雅的后端模块设计与实现，不方便的代码生成器扩展等问题，让作者有了开发一款自己的RuoYi项目想法。

blocks项目由主要分三大部分，包括：blocks-generator、blocks-rbac、blocks-ui。除blocks-generator外，其余子模块都可通过引入依赖的方式集成到其他项目中。
